...
import { mockProperties, mockPropertyReviews } from "@/data/mock";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: property ? `${property.title} | Klusta Admin` : "Property | Klusta Admin",
  };
}
...
export default async function PropertyDetailsPage({ params }: PageProps) {
  ...
  const reviews = mockPropertyReviews.slice(0, 4);
  const rating = property.rating ?? 4.7;
  const reviewCount = property.reviewCount ?? 1600;
    ...
    <div className="min-h-screen bg-white pb-24 dark:bg-gray-900">
      ...
      <div className="mx-auto max-w-(--breakpoint-2xl) px-4 py-6 md:px-6">
        ...
        <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-bold text-typography dark:text-white/90 sm:text-2xl">
              {property.title}
            </h1>
          ...
          <div className="flex flex-col items-start gap-1 sm:items-end">
            ...
            <p className="text-lg font-semibold text-primary dark:text-primary-50">
              {property.price}
            </p>
          </div>
        </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
              {property.amenities.map((name) => (
                ...
                >
                  ...
                  <span className="text-theme-xs font-medium text-typography dark:text-white/90">
                    {name}
                  </span>
                </div>
              ))}
        ...
        <section className="mt-8">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href="#"
                className="text-theme-sm font-medium text-primary hover:underline dark:text-primary-50"
              >
                View all
              </Link>
            </div>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {reviews.map((review) => (
              ...
              >
                ...
                <p className="mt-3 line-clamp-3 text-theme-sm text-gray-600 dark:text-gray-300">
                  {review.text}
                </p>
              </div>
            ))}
