...
export default function PropertyDetailContent({ id }: PropertyDetailContentProps) {
    ...
    return (
      <div className="min-h-screen bg-white pb-24 dark:bg-gray-900">
        <div className="mx-auto max-w-(--breakpoint-2xl) px-4 py-12 md:px-6">
          ...
          <Link
            href="/properties"
            className="mt-4 inline-flex items-center gap-2 text-theme-sm font-medium text-primary"
          >
  ...
  const reviews: { id: string; userName: string; userAvatar?: string; rating: number; timeAgo: string; text: string }[] = [];
  ...
  return (
    <div className="min-h-screen bg-white pb-24 dark:bg-gray-900">
      ...
      <div className="mx-auto max-w-(--breakpoint-2xl) px-4 py-6 md:px-6">
            ...
            {(property.rating != null || property.reviewCount != null) && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-theme-sm text-gray-600 dark:text-gray-300">
                  {rating} ({reviewCount >= 1000 ? `${(reviewCount / 1000).toFixed(1)}k` : reviewCount} reviews)
                </span>
              </div>
            )}
            <p className="text-lg font-semibold text-primary dark:text-primary-50">{property.price}</p>
          </div>
        </div>
        ...
        {(property.groundRules?.length ?? 0) > 0 && (
          <section className="mt-8">
        ...
        <section className="mt-8">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="#" className="text-theme-sm font-medium text-primary hover:underline dark:text-primary-50">
                View all
              </Link>
            </div>
          </div>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {reviews.length === 0 ? (
              <p className="col-span-2 text-theme-sm text-gray-500 dark:text-gray-400">No reviews yet.</p>
            ) : (
              reviews.map((review) => (
                <div
                  key={review.id}
        ...
        {property.homeownerName && (
          <section className="mt-8">
            <p className="text-theme-sm font-medium text-typography dark:text-white/90">{property.homeownerName}</p>
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-base font-semibold text-typography dark:text-white/90">Reviews to Homeowner</h2>
              <div className="flex items-center gap-2">
                <StarRating rating={rating} />
                <span className="text-theme-sm text-gray-600 dark:text-gray-300">
                  {rating} ({reviewCount >= 1000 ? `${(reviewCount / 1000).toFixed(1)}k` : reviewCount} reviews)
      ...
      <PropertyDetailsStickyBar
        ...
        status={property.status as "listed" | "unlisted" | "pending"}
